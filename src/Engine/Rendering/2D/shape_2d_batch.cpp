...
namespace Engine {
  ...
  void Shape2DBatch::DrawLine(float x0, float y0, float x1, float y1,
    ...
    entry.data.r = static_cast<float>(color.r) / 255.0f;
    entry.data.g = static_cast<float>(color.g) / 255.0f;
    entry.data.b = static_cast<float>(color.b) / 255.0f;
    entry.data.a = static_cast<float>(color.a) / 255.0f;
